...
ClusterManager::generateClusters()
{
	...
	kmeans.setDistanceType(Kmeans::EUCLID);
...
ClusterManager::generateClusters_Randomrestarts()
{
	...
	for(int i=0;i<restartCnt;i++)
	{
		...
		k->setDistanceType(Kmeans::EUCLID);
	...
	Kmeans* bestkmeans=kmset[bestkm_min];
	cout << "Best kmeans (max) found at " << bestkm_max <<" with dist " << maxDist << endl;
	cout << "Best kmeans (min) found at " << bestkm_min <<" with dist " << minDist << endl;
	char aFName[1024];
	sprintf(aFName,"%s/km_clusters_%d.txt",outputDir,clusterCnt);
