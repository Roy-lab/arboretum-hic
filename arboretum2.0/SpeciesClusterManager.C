...
SpeciesClusterManager::estimateMeanCov(Expert* e, string& specName, int clusterID)
{
	...
	//Get the prior covs with this species
	if(priorCovariance.find(specName)==priorCovariance.end())
	{
		cout <<"What! No prior cov found!!" << endl;
		exit(0);
	}
	map<int,Matrix*>* covsForSpecies=priorCovariance[specName];
	Matrix* covariance_prior=(*covsForSpecies)[clusterID];
	for(int i=0;i<dim;i++)
	{
		...
		for(int j=i;j<i+1;j++)
		{
			...
			cov=cov/sum;
			double prior_wt=sum/(sum+prior_cov);
			double cov_prior=covariance_prior->getValue(0,i);
			double cov_post=(prior_wt*cov)+((1-prior_wt)*cov_prior);
			...
			if(cov_post>1)
			{
				cout << "Big var: " << cov_post <<" prior " << cov_prior << " ml "<< cov << " prior_wt " << (1-prior_wt) << endl;
			}
		}
	}
...
int
SpeciesClusterManager::initCovFromGammaAllSpecies()
{
	prior_cov=0;//200;//0
	for(map<string,GeneExpManager*>::iterator aIter=speciesExprSet.begin();aIter!=speciesExprSet.end();aIter++)
        {
		GeneExpManager* exprMgr=aIter->second;
		map<int,Matrix*>* priorCovs=new map<int,Matrix*>;
		for(int k=0;k<maxClusterCnt;k++)
		{
			int dim=exprMgr->getGeneSet().begin()->second->size();
			Matrix* covariance=new Matrix(1,dim);
			for(int i=0;i<dim;i++)
			{
				double prec=gsl_ran_gamma(r,4,2);
				bool foundprec=false;
				double cov=0;
				//at some point we need to put additional checks to remove the possibility of an infinite loop
				while(!foundprec)
				{
					if(prec<2.0)
					{
						prec=gsl_ran_gamma(r,4,2);
					}
					else
					{
						foundprec=true;
				...
				cov=1/prec;
				covariance->setValue(cov,0,i);
			}
			(*priorCovs)[k]=covariance;
		}
		priorCovariance[aIter->first]=priorCovs;
		cout <<"Done initing prior covs for " << aIter->first << endl;
	}
	return 0;
}
