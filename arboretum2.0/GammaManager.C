...
GammaManager::estimateNonLeafPosteriorAlpha(Gamma::Node* node)
{
	...
	for(int r=0;r<maxClusterCnt;r++)
	{
		...
		if(node->leftchild!=NULL && node->leftchild->alpha!=NULL && node->rightchild!=NULL && node->rightchild->alpha!=NULL)
		{
			...
			//SR: clipping the alphas. For the leaf nodes we have a threshold of 1e-300. So for the parent/internals lets set it to 1e-100
...
int
GammaManager::showAllAlphas(Gamma::Node* node)
{
	if(node->alpha!=NULL)
	{
		cout <<"Alpha for " << node->name << endl;
		node->alpha->showMatrix();
	}
	if(node->leftchild!=NULL)
	{
		showAllAlphas(node->leftchild);	
	}
	if(node->rightchild!=NULL)
	{
		showAllAlphas(node->rightchild);
	}
	return 0;
}
